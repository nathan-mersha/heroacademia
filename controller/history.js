 ...
 * @name                heroAcademia
 * @module              history.js
 * @description         Controller for history
 * @kind                Controller
 * @copyright           Copyright : 2018
 */

let
    historyDAL        = require('../dal/history'),
    ...
    debug                   = require('debug')('heroAcademia/controller/history'),
    util                    = require('util'),
...
/**
 * @name                - Create
 * @description         - Creates history data
...
exports.create          = function (req, res, next) {
    ...
    function createData (callback) {
        ...
        historyDAL.create(body,function (err, data) { // Creating history data
            queryResponseHandler(err,data,res,function (err, data) {
                if(data){ // history data successfully created
                    res.status(201);
                    res.json(data);
                    callback(null);
                }else {
                    let errMsg = errorCodes.SEC.NO_DATA_FOUND;
                    errMsg.detail = 'history data could not be found (Failed to create history data)';
                    res.status(500);
                    res.json(errMsg);
                }
            });
...
exports.find            = function (req, res, next) {
    ...
    if(! util.isNullOrUndefined(req.query._id)) {
        ...
        let option = {
            ...
            select   : "angerLevel optimismLevel enemyDefeat archNemesis battleGroundDefeat enemyCapture alive firstModified lastModified"
        };

        let query = controllerHelper.queryFilter(req,["angerLevel","optimismLevel","enemyDefeat","archNemesis","battleGroundDefeat","enemyCapture","alive" , "_id", "__v"]);
        // get collection paginated invoked
        historyDAL.getCollectionsPaginated(query,option,function (err,data) {
            queryResponseHandler(err,data,res,function (err, data) { // Possible errors are handled.
                if(data){ // Data found.
                    ...
                    let errMsg = errorCodes.SEC.NO_DATA_FOUND;
                    errMsg.detail = "No history data found."; // Adding detailed data.
                    res.status(404);
                    res.json(errMsg);
                }
            });
...
/**
 * @name                - Update Many
 * @description         - Updates all history data that matches query
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
exports.update          = function (req, res, next) {
    ...
    function pickUpdateData (callback)                  {
        ...
        controllerHelper.pickUpdateData(["angerLevel","optimismLevel","enemyDefeat","archNemesis","battleGroundDefeat","enemyCapture","alive"],req,function (err,validUpdateData) { // Picking up valid update data.
    ...
    function updateData (validUpdateData,callback)  {
        ...
        if(Object.keys(query).length === 0){
             ...
             historyDAL.updateMany(query,validUpdateData,function (err,data) {   // handling update.
                 queryResponseHandler(err,data,res,function (err, data) {    // Possible errors are handled.
                     res.status(200);
                     res.send(data);
                     callback(null);
...
/**
 * @name                - Remove Many
 * @description         - Removes all history data that matches query
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
exports.remove          = function (req, res, next) {
    ...
    if(Object.keys(query).length === 0){
           ...
           historyDAL.removeMany(query,function (err,data) {   // handling update.
               queryResponseHandler(err,data,res,function (err, data) {   // Possible errors are handled.
                   res.status(200);
                   res.send(data);
               });
 ...
 *
 * @name                - Find public
 * @description         - Find history data by id visible only fields that are public.
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
function getPublic  (req, res, next) {
    ...
    let
        historyId   = req.query._id,
        query       = {_id: historyId}; // query construction.

    historyDAL.getPublic(query,function (err,data) { // retrieve history public data (with out the value)
        queryResponseHandler(err,data,res,function (err, data) { // Error handled.
            if(!data){ // No history value could be found
                res.status(404);
                res.json(errorCodes.SEC.NO_DATA_FOUND);
            }else if(data){ // Found history data
                res.status(200);
                res.send(data);
            }
        });
...
/**
 * @name                - Find private
 * @description         - Find history data by id visible only fields that are private.
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
function getPrivate (req, res, next) {
    ...
    historyDAL.getPrivate(query,function (err,data) { // retrieve history public data (with out the value)
        queryResponseHandler(err,data,res,function (err, data) { // Error handled.
            if(!data){ // No history value could be found
                res.status(404);
                res.json(errorCodes.SEC.NO_DATA_FOUND);
